/*
  出走小芽 Demo - DLight + IMU + OLED 文案逻辑预备版
  ...
  4. 光照控制植物情绪
  5. 运动控制植物动作
  6. 根据 光照状态 + 运动状态 生成 OLED 文案
  7. 当前先把 OLED 文案打印到 Serial Monitor 预览

  当前不接 OLED。
  等分线器到了，再把 OLED 文案真正画到 OLED 上。
*/
...
/*
  ...
  生成未来 OLED 上要显示的 4 行文字。

  当前只是生成文字，不真正显示到 OLED。
*/

OLEDText getOLEDText(LightMood lightMood, MotionState motion) {
  ...
  // ========== 2. 缺光 + 移动 ==========
  if (lightMood == LIGHT_NEED_SUN && motion == MOTION_ACTIVE) {
    return {
      "City",
      "Moving",
      "Are we",
  ...
  // ========== 3. 室内光 + 静止 ==========
  if (lightMood == LIGHT_INDOOR && motion == MOTION_STILL) {
    return {
      "City",
      "Indoor",
      "I am",
  ...
  // ========== 4. 室内光 + 移动 ==========
  if (lightMood == LIGHT_INDOOR && motion == MOTION_ACTIVE) {
    return {
      "City",
      "Carried",
      "Where are",
  ...
  // ========== 5. 明亮 + 静止 ==========
  if (lightMood == LIGHT_BRIGHT && motion == MOTION_STILL) {
    return {
      "City",
      "Bright",
      "This light",
  ...
  // ========== 6. 明亮 + 移动 ==========
  if (lightMood == LIGHT_BRIGHT && motion == MOTION_ACTIVE) {
    return {
      "City",
      "Walking",
      "I feel",
  ...
  // ========== 7. 强光 + 静止 ==========
  if (lightMood == LIGHT_SUNLIGHT && motion == MOTION_STILL) {
    return {
      "City",
      "Sunlight",
      "I found",
  ...
  // ========== 8. 强光 + 移动 ==========
  if (lightMood == LIGHT_SUNLIGHT && motion == MOTION_ACTIVE) {
    return {
      "City",
      "Outside",
      "We are",
...
/*
  当前没有接 OLED，所以先把未来 OLED 的内容打印到 Serial Monitor。

  等分线器到了之后，这个函数可以替换成：

  drawOLEDText(oledText);

  然后用 u8g2.drawStr() 真正画到 OLED 上。
*/

void printOLEDTextPreview(OLEDText text) {

  Serial.println();
  Serial.println("----- OLED TEXT PREVIEW -----");

  Serial.println(text.title);
  Serial.println(text.status);
  Serial.println("-----------------------------");
  Serial.println(text.line1);
  Serial.println(text.line2);
...
void setup() {
  ...
  M5.Display.print("Text preview");

  Serial.println();
  Serial.println("DLight + IMU + OLED text preview demo start");
  Serial.println("--------------------------------");
  ...
  Serial.println("--------------------------------");
  Serial.println("OLED text will be previewed in Serial Monitor.");
  Serial.println("--------------------------------");
}
...
void loop() {
  ...
  OLEDText oledText = getOLEDText(currentLightMood, currentMotion);


  // -----------------------------
  // 16.5 Serial Monitor 调试输出
  // -----------------------------
